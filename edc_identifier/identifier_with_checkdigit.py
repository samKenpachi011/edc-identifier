...
class IdentifierWithCheckdigit(Identifier):
    ...
    def __init__(self, last_identifier=None, **kwargs):
        if self.checkdigit_pattern:
            last_identifier = self.remove_checkdigit(
                last_identifier or self.last_identifier)
        super().__init__(last_identifier, **kwargs)
    ...
    def insert_separator(self, identifier, checkdigit=None):
        ...
        """
        identifier = super(
            IdentifierWithCheckdigit, self).insert_separator(identifier)
        identifier = (self.separator or '').join(
            [identifier, checkdigit] if checkdigit else [identifier])
        return identifier

    def remove_checkdigit(self, identifier_with_checkdigit):
        """Returns the identifier, less the check digit.

        * If checkdigit_pattern is None, does nothing and just
          returns parameter `identifier_with_checkdigit`.
        * Raises an exception if the patterns do not match or the
          checkdigit is invalid.
        """
        identifier = None
        if self.checkdigit_pattern and identifier_with_checkdigit:
            try:
                checkdigit = re.findall(
