...
class ResearchIdentifier(LuhnMixin):
    ...
    def __init__(self, object_to_identify=None, requesting_model=None, template=None, identifier=None, **kwargs):
        self.object_to_identify = object_to_identify  # e.g. 'subject', 'infant', 'plot', a.k.a subject_type
    ...
    def create(self, **kwargs):
        """Creates a new and unique identifier and updates the IdentifierModel."""
        ...
        for field in fields:
            try:
                if not template_options[field]:
                    raise self.exception('Required option not provided. Got {}'.format(field))
            except KeyError:
                raise self.exception('Required option not provided. Got {}'.format(field))
